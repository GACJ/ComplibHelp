...
When a method is added to Complib, various properties of the method are determined in order to classify it. These properties are displayed in the method page's **properties tab**.
...
### Changes
A method consists of a sequence of **rows** rung in sequence. The relationship between two adjacent rows with regard to how bells move from one change to the other is referred to as a **change**. There are a number of different types of change that can be made between adjacent rows of a method:

Adjacent
:   The method only contains changes where bells move at most one place between rows. All traditional methods have changes of this type.
...
A method with a single non-stationary hunt bell will fall into one and only one hunter class (\* a method may be Single and/or Little in addition to its main classification.)
...
### Leadheads
Almost all methods (currently all methods in the CCCBR Library) have a plain course which can be divided into one or more sections of fixed, finite length, which are called **leads** or **divisions**. It is common to refer to the starting row of a lead (or division) as the **leadhead** (or **division end**). The collected set of leadheads of a method may satisfy one of a number of conditions:
...
Uneven parity
:   The plain course contains an unequal number of **odd** and **even** rows (see Row Parity).
...
Default calls
:   The **call type** which will be used by default when creating a composition for the method. The call types that Complib recognises are:
...
Method above/below
:   For methods which are hunters, if the method shares **above/below work** with another method that was rung prior to it, that method will be listed here.

FCH Groups
:   Lists the codes for the method's **false coursehead groups** (if applicable).

Accepted extension path
:   For methods with a compliant **extension** which has been named, lists the extension path(s) used to derive the accepted extension.

Compositions designed for this method
:   The number of (published) compositions on Complib which include this method in their definition.
...
Lead length
:   The number of rows in a plain lead.

Plain course length
:   The number of rows in the method's plain course. This is equal to the number of leads in the cours multiplied by the lead length.

Hunt bells
:   The number of hunt bells (see [Hunt bells](#hunt-bells)).
